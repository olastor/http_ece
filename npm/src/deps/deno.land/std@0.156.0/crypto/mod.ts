...
const bufferSourceBytes = (data: dntShim.BufferSource | unknown) => {
...
export interface StdSubtleCrypto extends dntShim.SubtleCrypto {
  ...
  digest(
    ...
    data: dntShim.BufferSource | AsyncIterable<dntShim.BufferSource> | Iterable<dntShim.BufferSource>,
  ): Promise<ArrayBuffer>;
  ...
  digestSync(
    algorithm: DigestAlgorithm,
    data: dntShim.BufferSource | Iterable<dntShim.BufferSource>,
  ): ArrayBuffer;
  ...
  subtle: {
    ...
    async digest(
      ...
      if (
        ...
        } else if ((data as Iterable<dntShim.BufferSource>)[Symbol.iterator]) {
          return stdCrypto.subtle.digestSync(
            algorithm,
            data as Iterable<dntShim.BufferSource>,
          );
        } else if (
          (data as AsyncIterable<dntShim.BufferSource>)[Symbol.asyncIterator]
        ) {
          const wasmCrypto = instantiateWasm();
          const context = new wasmCrypto.DigestContext(name);
          for await (const chunk of data as AsyncIterable<dntShim.BufferSource>) {
    ...
    digestSync(
      ...
      if (bytes) {
        ...
        for (const chunk of data as Iterable<dntShim.BufferSource>) {
          const chunkBytes = bufferSourceBytes(chunk);
          if (!chunkBytes) {
            throw new TypeError("data contained chunk of the wrong type");
          }
