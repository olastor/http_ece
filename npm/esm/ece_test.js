...
dntShim.Deno.test("encrypt/RFC8188/Example1", async () => {
    ...
    assert(bytes.equals(new Uint8Array(result), base64url.decode("I1BsxtFttlv3u_Oo94xnmwAAEAAA-NAVub2qFgBEuQKRapoZu-IxkIva3MEB1PD-ly8Thjg")));
});
dntShim.Deno.test("encrypt/RFC8188/Example2", async () => {
    ...
    assert(bytes.equals(new Uint8Array(result), base64url.decode("uNCkWiNYzKTnBN9ji3-qWAAAABkCYTHOG8chz_gnvgOqdGYovxyjuqRyJFjEDyoF1Fvkj6hQPdPHI51OEUKEpgz3SsLWIqS_uA")));
});
dntShim.Deno.test("decrypt/RFC8188/Example1", async () => {
    ...
    assert(bytes.equals(new Uint8Array(result), base64url.decode("SSBhbSB0aGUgd2FscnVz")));
});
